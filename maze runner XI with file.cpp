...
ifstream maze_input("maze_input.txt",ios::in);  //Syntax for creating file
...
void build_graph()
{
    ...
    do
    {
        ...
        maze_input>>p>>q;  //taking input from file
        if(p==0&&q==0)
        {
            break;
        }
...
void maze(int i,int j,int m,int n)
{
    ...
    while(1)
    {
        ...
        else if(adj[i][j+1]!=1&&adj[i][j-1]!=1&&adj[i-1][j]!=1&&adj[i+1][j]!=1&&s.size()!=0)  //backtracking condition applies here
        {
...
int main()
{
    ...
    cout<<"\t\t\t\t Maze Runners\n\n\n";
    if(!maze_input)  //If file is not open
    {
    ...
    maze_input>>v;  //Taking Input from file
    cout<<v<<endl<<endl;
